...
import { useBudget, useProducts } from "@/lib/db";
...
function BudgetPage() {
  ...
  const { data: budget, isLoading: bLoading } = useBudget(id);
  const { items: products, loading } = useProducts();

  const filtered = useMemo(() => {
    if (!budget) return [];
    return products.filter((p) => p.price <= budget.maxAmount);
  }, [products, budget]);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">
              Under {bLoading ? "…" : budget?.label ?? "this budget"}
            </h1>
          </div>
          ...
          {loading || bLoading ? (
            ...
            <div className="text-center py-20 text-sm text-gray-500 rounded-3xl backdrop-blur-xl bg-white/60 border border-white/60">
              No products under this budget yet. Check back soon!
            </div>
          ) : (
